...
class OutputFormatter:
    def __init__(self, output_file=None, verbose=False, quiet=False, format="json"):
        ...
        self.findings = {'found': 0, 'forbidden': 0, 'redirects': 0, 'unauthorized': 0}
    ...
    def recon_summary(self, recon_data):
        ...
        print(f"{Fore.CYAN}[*] {Fore.WHITE}Reconnaissance Summary{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'═' * 80}{Style.RESET_ALL}\n")
        print(f"{Fore.GREEN}[+] {Fore.WHITE}URL:{Style.RESET_ALL} {Fore.CYAN}{recon_data.get('url', 'Unknown')}{Style.RESET_ALL}")
        print(f"{Fore.GREEN}[+] {Fore.WHITE}Status:{Style.RESET_ALL} {Fore.GREEN}{recon_data.get('status_code', 0)}{Style.RESET_ALL}")
        print(f"{Fore.GREEN}[+] {Fore.WHITE}Response Time:{Style.RESET_ALL} {Fore.YELLOW}{recon_data.get('response_time', 0):.2f}s{Style.RESET_ALL}")
        if recon_data.get('page_title'):
            print(f"{Fore.GREEN}[+] {Fore.WHITE}Page Title:{Style.RESET_ALL} {recon_data.get('page_title')[:60]}")
        tech_list = recon_data.get('tech', [])
        if tech_list:
            print(f"\n{Fore.GREEN}[+] {Fore.WHITE}Technologies Detected:{Style.RESET_ALL}")
            for i, tech in enumerate(tech_list[:10], 1):
                print(f"    {Fore.YELLOW}[{i:>2}]{Style.RESET_ALL} {tech}")
        print(f"\n{Fore.CYAN}{'═' * 80}{Style.RESET_ALL}\n")
    ...
    def found(self, path, status, size, response_time, content_type):
        ...
        if not self.quiet:
            if status == 200:
                ...
                symbol = f"{Fore.MAGENTA}[!]{Style.RESET_ALL}"
                status_str = f"{Fore.MAGENTA}{status}{Style.RESET_ALL}"
            else:
                symbol = f"{Fore.WHITE}[*]{Style.RESET_ALL}"
                status_str = f"{Fore.WHITE}{status}{Style.RESET_ALL}"
            if size < 1024:
                size_str = f"{size}B"
            elif size < 1024 * 1024:
            ...
            print(f"{symbol} {path:<50} [Status: {status_str}] [Size: {Fore.CYAN}{size_str}{Style.RESET_ALL}]")
    ...
    def summary(self, results):
        ...
        print(f"{Fore.CYAN}{scan_complete:^80}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'':^80}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'═' * 80}{Style.RESET_ALL}\n")
        ...
        total_tested = results.get('total_paths_tested', 0)
    ...
    def _save_json(self, results):
        output_data = {'metadata': {'tool': 'AIBuster', 'version': '2.5.0', 'timestamp': datetime.now().isoformat()}, 'results': results}
